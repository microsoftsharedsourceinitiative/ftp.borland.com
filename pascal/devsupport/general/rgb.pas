...
begin
  ...
  { Notice how with the current palette settings, only the text for
    "Press any key...", "Black", "Light Gray", and "White" are visible.
    This occurs as the palette entry for color 0 (Black) has been set to
    display as white.  For the text "Light Gray" and "White", it happens
    that color 0 (Black) is used at the background.
  }
  SetColor(0);
  ...
  { Restore original colors to the palette.
    The colors may vary from the default depending upon initial
    values used by your video system.
  }
  for i := 0 to 15 do
    with Colors[i] do
      SetRGBPalette(ColorNum, RedVal,GreenVal,
                    BlueVal);
